...
def verifyForHaemoglobinMention(sentence):
    
    global hgbPattern
    if len(hgbPattern) > 0:
        for hgbSynonym in hgbPattern:
            sentence = re.sub(hgbSynonym, ' hemoglobin ', sentence, flags=re.RegexFlag.IGNORECASE)
    return(sentence)
    ...
    for splitSubRecords in splitBatchRecords:
        if not (str(splitSubRecords).isspace()):
            ...
            for splitSentence in bundledSentence:
                ...
                splitSentence = verifyForHaemoglobinMention(splitSentence)
                ''' dictionary data'''
...
def getCrClearanceValue(sentence):
    ...
    if (testValue > rangeValues[0]) and (testValue < rangeValues[1]):
...
def isolateCrCandidateInstances(currentDocumentRecord, triggerDominatedSentiment, recordCandidateInstances, modelId):
    ...
    for sentence in currentDocumentRecord:
        ...
        if (len(startIndexList)>0) and (len(endIndexList)>0):
            ...
            valueList = list(filter(lambda token : re.search('\d+\.\d+', token, flags=re.RegexFlag.IGNORECASE),sentenceTokens))
            if len(valueList) > 0:
                sentence = ' '.join(word.strip() for word in sentenceTokens)
                #print("sentence b4>>>",sentence)
                retTestStatus = getCrClearanceValue(sentence)
                sentence = 'hemoglobin value is '+str(retTestStatus)
            else:
                sentence=''
            if len(sentence) > 0:
...
def generateTokenBasedFrequency(candidateRecordsDictionary):
    ...
    for filename in candidateRecordsDictionary.keys():
        ...
        for sentence in decoyList:
            if sentence is not 'hemoglobin record not found':
...
if TRIGGER_ATTRIBUTE == 'HBA1C':
    hbPseudoFile = "".join([metaDataPath,"/",'hbTestPseudonyms'])
    with open(hbPseudoFile) as readBuffer:
        currentLine = readBuffer.readline()
        while currentLine:
            startAppend = ''
            terminalAppend = ''
            hgbSynonym = re.sub('\n', '', currentLine)
            hgbSynonym = buildMatternPattern(hgbSynonym,startAppend,terminalAppend)
            hgbPattern.append(hgbSynonym)
            currentLine = readBuffer.readline() 
...
for filename in list(listedPatientRecords):
        ...
        if (len(tier1RecordCandidateInstances)==0):
            tier1RecordCandidateInstances = ['hemoglobin record not found']
