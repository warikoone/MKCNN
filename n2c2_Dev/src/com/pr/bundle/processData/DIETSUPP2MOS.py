...
def populateArrayValueDictionary(tempDict,key,updateValue):
    
    tempValueArray=[] 
    if key in tempDict.keys():
        tempValueArray = tempDict[key]
    tempValueArray.append(updateValue)
    tempDict.update({key:tempValueArray})

def verifyTimeThreshold(record,dateDictionary,timeThresholdCoverage):
    
    currStatus = True
    nextStatus = True
    dateMatcher = re.search('(\w+)\:\s\d{4}(\-\d{2})+', record, flags=re.RegexFlag.IGNORECASE)
    if dateMatcher:
        dateValue = dateMatcher.group(0)
        dateValue = re.split('\:', dateValue)[1]
        dateValue = re.split("\-", dateValue)
        dateDictionary.append(datetime(int(dateValue[0]),int(dateValue[1]),int(dateValue[2])))
    else:
        return(False,True)
    #print("dateDictionary>>",dateDictionary)   
    if len(dateDictionary) > 1:
        latest = dateDictionary[0]
        prior = dateDictionary[(len(dateDictionary)-1)]
        #print("lstest>>",latest,"\t prior>>",prior)
        currScore = relativedelta.relativedelta(latest,prior)
        yearScore = (currScore.years*12)
        monthScore = currScore.months
        dateScore = int(np.rint(currScore.days/30))
        finalScore = yearScore+monthScore+dateScore
        if finalScore in timeThresholdCoverage:
            currStatus = True
            nextStatus = False
        else:
            if finalScore < np.min(timeThresholdCoverage):
                currStatus = True
                nextStatus = True
            elif finalScore > np.max(timeThresholdCoverage):
                currStatus = True
                nextStatus = False
                
        #print("final Score:::",currScore)
    #print(" chek value>>",currStatus,"\t>",nextStatus)
    return(currStatus,nextStatus)

def checkForMedications(bundledSentence):
    
    returnBundle = []
    reBundleStatus = False
    pivotTerm = 0
    for eachBundle in bundledSentence:
        medicationMatcher = re.search('(^|\s)med', eachBundle, flags=re.RegexFlag.IGNORECASE)
        if medicationMatcher:
            reBundleStatus = True
            pivotTerm = 1
        else:
            pivotTerm = 0
        ...
        if reBundleStatus:
            if ((re.search('\:', eachBundle, flags=re.RegexFlag.IGNORECASE)) and (pivotTerm == 0)):
                reBundleStatus = False
            else:
                returnBundle.append(eachBundle)
    return(returnBundle)
...
def extractVisitRecords(recordBatch):
    ...
    sentenceRecord = []
    dateMention = []
    timeThresholdCoverage = range(2,13)
    splitBatchRecords.reverse()
    for splitSubRecords in splitBatchRecords:
        if not (str(splitSubRecords).isspace()):
           # print("date>>>",dateMention)
            #print(">>",count)
            currStatus,nextStatus = verifyTimeThreshold(splitSubRecords,dateMention,timeThresholdCoverage)
            if(currStatus):
                #print(">>>",splitSubRecords)
                bundledSentence = removeNewlines(splitSubRecords,'\n')
                bundledSentence = checkForMedications(bundledSentence)
                for splitSentence in bundledSentence:
                    #print("new >>>",splitSentence)
                    splitSentence = removeStopWords(splitSentence)
                    splitSentence = performSentenceSplit(splitSentence)
                    ''' dictionary data'''
                    tokenIndexMapper.extend(splitSentence)
                    ''' sentence record'''
                    #sentenceRecord.append(removeStopWords(splitSentence))
                    sentenceRecord.extend(splitSentence)
                    #break
            if (nextStatus):
                count+=1
            else:
...
def generateTokenBasedFrequency(candidateRecordsDictionary):
    ...
    for filename in candidateRecordsDictionary.keys():
        ...
        for sentence in decoyList:
            if sentence is not 'no suppliments prescribed':
    ...
    wordIDFDict = dict(filter(lambda wordScore : wordScore[1]>1, wordIDFDict.items()))
    print("3>>>",wordIDFDict)
    return()     
...
for filename in list(listedPatientRecords):
        ...
        for triggerIndex,triggerValue in enumerate(TRIGGER_FEATURE):
            ...
            #print("\t>",triggerValue,"\t>>",decoyDictionaryList)
            tier1DictionaryList.append(decoyDictionaryList[0])
        ...
        if len(tier1DictionaryList) > 0:
            threshold = [0.7,0.48,0.31]
            if filename in testRecords:
                #threshold = list(map(lambda threshValue: (threshValue-0.05),threshold))
                threshold = [0.65,0.48,0.31]
            #print(">>>",tier1DictionaryList)
            for index,listEntry in enumerate(tier1DictionaryList):
                #print("\n",index,"\t>>",listEntry,"\t>>",threshold[index])
                if (index == 2):
                    tempMatch = dict(filter(lambda pattern : re.match('quinine', pattern[0].strip(), flags=re.RegexFlag.IGNORECASE),listEntry.items()))
                    #print("temp>>",tempMatch)
                    if len(tempMatch) > 0:
                        tier1TriggerDominatedSentiment.extend(list(tempMatch.keys()))
                    
                listEntry = dict(filter(lambda itemKey : itemKey[1] > threshold[index], listEntry.items()))
                #print("\n",index,"\t>>",listEntry,"\t>>",threshold[index])
                tier1TriggerDominatedSentiment.extend(list(listEntry.keys()))
            tier1TriggerDominatedSentiment = set(tier1TriggerDominatedSentiment)
        ...
        if len(tier1TriggerDominatedSentiment) > 0:
            ...
            tier1RecordCandidateInstances = ['no suppliments prescribed']
