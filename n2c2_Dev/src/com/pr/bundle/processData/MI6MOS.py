...
def extractVisitRecords(recordBatch):
    ...
    sentenceRecord = []
    dateMention = []
    timeThresholdCoverage = range(6,12)
...
def generateTokenBasedFrequency(candidateRecordsDictionary):
    ...
    for filename in candidateRecordsDictionary.keys():
        ...
        for sentence in decoyList:
            if sentence is not 'does not have myocardial infarction':
...
for filename in list(listedPatientRecords):
        ...
        seedIndex=0
        if(len(tier1DictionaryList[seedIndex]) > 0):
            VARIABLE_ATTRIBUTE_THRESHOLD = 0.6
            if filename in testRecords:
                VARIABLE_ATTRIBUTE_THRESHOLD = 0.56
        ...
        if len(tier1TriggerDominatedSentiment)>0:
            ...
            tier1RecordCandidateInstances = ['does not have myocardial infarction']
