...
def verifyForCreatinineMention(sentence):
    
    sentence = re.sub('\W+cr\W+|\W+creat\W+|\W+crit\W+|\W+CRE\W+', ' creatinine ', sentence, flags=re.RegexFlag.IGNORECASE)
    return(sentence)
    ...
    for splitSubRecords in splitBatchRecords:
        if not (str(splitSubRecords).isspace()):
            ...
            for splitSentence in bundledSentence:
                ...
                splitSentence = verifyForCreatinineMention(splitSentence)
                ''' dictionary data'''
...
def getCrClearanceValue(sentence):
    
    patternScreened = False
    retTestStatus = 'not sufficient'
    rangeValues = [0.6,1.5]
    clearancePattern = ['\d+\.\d+\-\d+\.\d+','\d+\.\d+','\d+']
    for index,patternType in enumerate(clearancePattern):
        hgbIterMatcher = re.finditer(patternType, sentence, flags=re.RegexFlag.IGNORECASE)
        for eachHgbIter in hgbIterMatcher:
            patternScreened = True
            if (index == 0):
                rangeValues = np.sort(np.array(re.split('-',eachHgbIter.group().strip())).astype(float))
                testResult = sentence[0:eachHgbIter.start()]
                valueMatch = re.search('\d+\.\d+', testResult, flags=re.RegexFlag.IGNORECASE)
                if valueMatch:
                    testValue = float(valueMatch.group().strip())
                else:
                    testValue = rangeValues[0]
                    rangeValues = [0.6,1.5]
                    print("\n\t SYTEM ERROR")
                    #exit()
            else:
                testValue = float(eachHgbIter.group().strip())
            break
        if patternScreened:
            break
    #print("test value>>>",testValue)
    if (testValue > rangeValues[1]):
        retTestStatus = testValue
...
def isolateCrCandidateInstances(currentDocumentRecord, triggerDominatedSentiment, recordCandidateInstances, modelId):
    ...
    global SENTENCE_TRIM_LIMIT,MAX_SEQUENCE_LENGTH
    for sentence in currentDocumentRecord:
        ...
        if (len(startIndexList)>0) and (len(endIndexList)>0):
            ...
            for index in range(len(startIndexList)):
                startIndex = startIndexList[index]
                endIndex = endIndexList[index]
                #print("limits\t >>",startIndex,'\t>>',endIndex)
                #print("\t >>",sentenceTokens[startIndex:endIndex+1])
                decoyTokens = list(filter(lambda token : re.match('[^\W]', token.strip(), flags=re.RegexFlag.IGNORECASE), sentenceTokens[startIndex:endIndex+1]))
                #sentenceTokens = sentenceTokens[startIndex:endIndex+1]
                if(len(decoyTokens) > MAX_SEQUENCE_LENGTH[modelId]):
                    MAX_SEQUENCE_LENGTH[modelId] = len(decoyTokens)
                valueList = list(filter(lambda token : re.search('\d+\.\d+|\d+', token, flags=re.RegexFlag.IGNORECASE),decoyTokens))
                if len(valueList) > 0:
                    sentence = ' '.join(word.strip() for word in decoyTokens)
                    #print("sentence b4>>>",sentence)
                    retTestStatus = getCrClearanceValue(sentence)
                    sentence = 'creatinine value is '+str(retTestStatus)
                else:
                    sentence=''
                if len(sentence) > 0:
...
def generateTokenBasedFrequency(candidateRecordsDictionary):
    ...
    for filename in candidateRecordsDictionary.keys():
        ...
        for sentence in decoyList:
            if sentence is not 'creatinine record not found':
...
for filename in list(listedPatientRecords):
        ...
        tier1TriggerDominatedSentiment = [TRIGGER_FEATURE]
        ...
        if (attributeExistStatus[TRIGGER_ATTRIBUTE] != 'na'):
            if ((filename in listedPatientRecords) and (attributeExistStatus[TRIGGER_ATTRIBUTE] == 'met')):
                for sentence in tier1RecordCandidateInstances:
                    if sentence.endswith('sufficient'):
                        tier1RecordCandidateInstances.remove(sentence)
        
        
        if (len(tier1RecordCandidateInstances)==0):
            tier1RecordCandidateInstances = ['creatinine record not found']
