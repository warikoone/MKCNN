...
def updateRenalReferences(sentence):
    
    sentence = re.sub('\W*renal\W*insufficiency\W*|\W*renal\W*failure\W*', ' nephropathy ', sentence, flags=re.RegexFlag.IGNORECASE)
    ...
    for splitSubRecords in splitBatchRecords:
        if str(splitSubRecords).isspace():
            ...
            for splitSentence in bundledSentence:
                ...
                splitSentence = updateRenalReferences(splitSentence)
                ''' dictionary data'''
...
def generateTokenBasedFrequency(candidateRecordsDictionary):
    ...
    for filename in candidateRecordsDictionary.keys():
        ...
        for sentence in decoyList:
            if sentence is not 'no indication of major diabetic problem':
...
for filename in list(listedPatientRecords):
        ...
        for triggerIndex,triggerValue in enumerate(TRIGGER_FEATURE):
            ...
            if(len(tempDictionaryList) > 0):
                VARIABLE_ATTRIBUTE_THRESHOLD = [0.6,0.6]
                if filename in testRecords:
                    VARIABLE_ATTRIBUTE_THRESHOLD = [0.59,0.59]
        ...
        tier1TriggerDominatedSentiment.extend(['nephropathy'])    
        tier1TriggerDominatedSentiment = list(set(tier1TriggerDominatedSentiment))    
        ...
        if ((len(tier1TriggerDominatedSentiment) == 0) or (len(tier1RecordCandidateInstances)==0)):
            tier1RecordCandidateInstances = ['no indication of major diabetic problem']
        
        '''
        currCount = 0
        for sentence in tier1RecordCandidateInstances:
            currCount = len(list(performWordSplit(sentence)))
            if(currCount > MAX_SEQUENCE_LENGTH[triggerIndex]):
                MAX_SEQUENCE_LENGTH[triggerIndex] = currCount
        '''
