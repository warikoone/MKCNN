...
def checkForAspirinCondition(sentence):
    ...
    if len(aspirinPattern) > 0:
        for aspirinSynonym in aspirinPattern:
            sentence = re.sub(aspirinSynonym, ' ', sentence, flags=re.RegexFlag.IGNORECASE)
    return(sentence)
...
def isolateCandidateInstances(currentDocumentRecord, triggerDominatedSentiment, recordCandidateInstances,modelId):
    ...
    for sentence in currentDocumentRecord:
        ...
        if (len(startIndexList)>0) and (len(endIndexList)>0):
            ...
            if (re.search('\d+', sentence, flags=re.RegexFlag.IGNORECASE)):
                sentence =''
...
def generateTokenBasedFrequency(candidateRecordsDictionary):
    ...
    for filename in candidateRecordsDictionary.keys():
        ...
        for sentence in decoyList:
            if sentence is not 'patient can manage independently':
...
for filename in list(listedPatientRecords):
    #if filename in ["100.xml","378.xml","380.xml"]:
        fieldIndex += 1
        ...
        for triggerIndex,triggerValue in enumerate(TRIGGER_FEATURE):
            tier1DictionaryList = []
            tier1WordList.extend(screenForTriggerFeatures(wordIndexMapper, tier1DictionaryList, triggerValue,triggerIndex))
            ...
            if(len(tier1DictionaryList) > 0):
                ...
                decoyDictionaryList = dict(filter(lambda itemPair : itemPair[1]> VARIABLE_ATTRIBUTE_THRESHOLD[triggerIndex],dict(tier1DictionaryList[0]).items())).keys()
                #decoyDictionaryList = list(filter(lambda trigger : not re.search('family', trigger, flags=re.RegexFlag.IGNORECASE),decoyDictionaryList))
                    
                tier1TriggerDominatedSentiment.extend(decoyDictionaryList)
        ...
        if (len(tier1TriggerDominatedSentiment)==0) or (len(tier1RecordCandidateInstances)==0):
            tier1RecordCandidateInstances = ['patient can manage independently']
