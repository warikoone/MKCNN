...
def generateTokenBasedFrequency(candidateRecordsDictionary):
    ...
    for filename in candidateRecordsDictionary.keys():
        ...
        for sentence in decoyList:
            if ((sentence is not 'does not have myocardial infarction') and (sentence is not 'aspirin not allowed')):
        ...
        if len(decoyTokens) > 0:
            ...
            for token in decoyTokens:
                ...
                count=0
                if token in decoyWordDict.keys():
                    count = decoyWordDict.get(token)
                count += 1
                decoyWordDict.update({token:count})
    
    print("1>>>",decoyWordDict)
    docSize = len(tier1CandidateRecordsDictionary)
    for item in decoyWordDict.items():
        if item[1] != 1:
...
#printRecords(tier2CandidateRecordsDictionary,tier2CandidateRecordsDecisionDictionary)
...
#generateTokenBasedFrequency(tier2CandidateRecordsDictionary)
