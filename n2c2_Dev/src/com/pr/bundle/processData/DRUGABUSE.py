...
def  drugAddictionValidation(decoyCandidateInstances):
    
    removeIndex = []
    for index,sentence in enumerate(decoyCandidateInstances):
        #print("\n\t>>",sentence, index)
        if(re.search('drug', sentence, flags=re.RegexFlag.IGNORECASE)):
            groupMatchStatus = False
            groupMatch = re.finditer('(?<=drugs)(\sabuse|\suse)|(?<=drug)(\sabuse|\suse)',sentence, flags=re.RegexFlag.IGNORECASE)
            for eachMatch in groupMatch:
                if eachMatch.group() != '':
                    groupMatchStatus = True
            #print(">>",sentence,">>>",groupMatchStatus)
            if not groupMatchStatus:
                removeIndex.append(index)
    if len(removeIndex) > 0:
        removeIndex.reverse()
        #print("pop val>>",removeIndex)
        for indexValue in removeIndex:
            #print(">>",decoyCandidateInstances[indexValue])
            decoyCandidateInstances.pop(indexValue)
            
    return(decoyCandidateInstances)
...
def generateTokenBasedFrequency(candidateRecordsDictionary):
    ...
    for filename in candidateRecordsDictionary.keys():
        ...
        for sentence in decoyList:
            if sentence is not 'no identified drug abuse':
    ...
    print("1>>>",wordIDFDict)
    docSize = len(candidateRecordsDictionary)
...
for filename in list(listedPatientRecords):
        ...
        if len(tier1TriggerDominatedSentiment) > 0:
            ...
            tier1RecordCandidateInstances = drugAddictionValidation(tier1RecordCandidateInstances)
            
        if((len(tier1TriggerDominatedSentiment)==0) or (len(tier1RecordCandidateInstances)==0)):
            tier1RecordCandidateInstances = ['no identified drug abuse']
